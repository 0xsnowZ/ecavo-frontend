        ...
        <aside className="lg:w-64 shrink-0">
          <div className="card p-5">
            ...
            <div className="flex flex-col items-center gap-2 text-center pb-5 border-b border-border mb-4">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-medium ${user?.role === "admin" ? "bg-primary/10 text-primary" : "bg-gray-100 text-gray-600"}`}
              >
                {user?.role === "admin"
                  ? t("account_page.role_admin")
                  : t("account_page.role_customer")}
              </span>
            </div>
            ...
            <div className="card overflow-hidden">
              <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                <h2 className="font-bold text-secondary">
                  {t("account_page.order_history")}
                </h2>
                <span className="text-xs text-muted">
                  {orders.length}{" "}
                  {orders.length === 1
                    ? t("account_page.order")
              ...
              {ordersLoading ? (
                ...
                <div className="text-center py-16">
                  ...
                  <Link to="/products" className="btn-primary mt-4 text-sm">
                    {t("account_page.shop_now")}
                  </Link>
                </div>
              ) : (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span
                              className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${STATUS_COLOR[order.status] || "bg-gray-100 text-gray-600"}`}
                            >
                              {t(`orders.status_${order.status}`, {
                                defaultValue: order.status.replace(/_/g, " "),
                              })}
                            </span>
                            <span className="text-xs text-muted">
                              {order.items?.length}{" "}
                              {t("account_page.items_count")}
                            </span>
                          </div>
                        </div>
                  ...
                  {editMode ? (
                    ...
                    <div className="flex items-center gap-2 py-2.5 px-3 bg-surface rounded-lg text-sm">
                      ...
                      <span className="text-dark">
                        {user?.phone || t("account_page.not_set")}
                      </span>
                    </div>
                  )}
                </div>
