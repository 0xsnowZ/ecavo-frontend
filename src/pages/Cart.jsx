...
export default function Cart() {
  ...
  const { items, removeItem, updateQty, getSubtotal, getTotal, deliveryFee,
          coupon, applyCoupon, removeCoupon, getDiscount } = useCartStore();
  ...
  const handleCoupon = async () => {
    ...
    setCouponError('');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/cart/apply-coupon`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setCouponError(data.message || (isAr ? 'كود غير صالح' : 'Invalid coupon'));
      } else {
        applyCoupon({ code: data.coupon_code, type: 'applied', value: data.discount });
      }
    } catch {
  ...
  if (items.length === 0) return (
    <div className="container-main py-24 flex flex-col items-center text-center gap-4">
      ...
      <p className="text-muted text-sm">
        {isAr ? 'أضف بعض المنتجات وابدأ التسوق!' : 'Add some products and start shopping!'}
      </p>
  ...
  return (
    <div className="container-main py-8">
      <h1 className="text-2xl font-bold text-secondary mb-6">{t('cart.title')}</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="hidden sm:grid grid-cols-12 text-xs text-muted uppercase tracking-wider px-4 pb-2 border-b border-border">
            ...
            <span className="col-span-2 text-center">{isAr ? 'السعر' : 'Price'}</span>
            <span className="col-span-2 text-center">{t('cart.qty')}</span>
            <span className="col-span-2 text-center">{isAr ? 'الإجمالي' : 'Total'}</span>
          </div>

          {items.map(item => {
            const price = item.product.price + (item.variant?.extra_price ?? 0);
            return (
              <div key={item.key} className="card p-4 grid grid-cols-12 gap-3 items-center animate-fade-in">
                {/* Image + name */}
                <div className="col-span-12 sm:col-span-6 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {/* Mobile price */}
                    <p className="sm:hidden text-sm font-bold text-primary mt-1">{fmt(price)}</p>
                  </div>
                </div>

                {/* Price */}
                <div className="hidden sm:flex col-span-2 justify-center">
                  <span className="text-sm font-semibold text-dark">{fmt(price)}</span>
                </div>
                ...
                <div className="col-span-4 sm:col-span-2 flex items-center justify-end sm:justify-center gap-2">
                  <span className="text-sm font-bold text-primary">{fmt(price * item.qty)}</span>
                  <button
          ...
          {/* Continue shopping */}
          <Link to="/products" className="inline-flex items-center gap-2 text-sm text-muted hover:text-primary transition-colors mt-2">
            ...
            <div>
              ...
              {coupon ? (
                <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  ...
                  <span className="text-sm font-semibold text-green-700 flex-1">{coupon.code}</span>
